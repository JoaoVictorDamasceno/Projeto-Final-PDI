...
05_plot_results.py
====================
Gera reproduções das Figuras 3 e 4 do artigo a partir do CSV de resultados
produzido por 04_benchmark.py.

Figura 3: comparação de FPS (Base FP32 vs FP16) por variante (nano/medium/xl)
          e por arquitetura (YOLOv8/v9/v11), com o percentual de ganho.
Figura 4: trade-off mAP@0.5 x FPS para as configurações quantizadas em FP16.

Uso:
    python 05_plot_results.py --csv /home/claude/polyp_yolo_quant/results/table1_results.csv \
        --outdir /home/claude/polyp_yolo_quant/results
"""
...
def plot_fps_comparison(df: pd.DataFrame, outpath: Path):
    ...
    fig.suptitle("Comparação da velocidade de inferência (FPS) entre YOLOv8, YOLOv9 e YOLOv11")
    fig.tight_layout()
    fig.savefig(outpath, dpi=200)
    plt.close(fig)
    print(f"Figura 3 salva em: {outpath}")
...
def plot_tradeoff(df: pd.DataFrame, outpath: Path):
    ...
    ax.set_title("Trade-off entre eficácia diagnóstica (mAP@0.5) e FPS — variantes FP16")
    ax.legend(loc="lower right", fontsize=8)
    fig.tight_layout()
    fig.savefig(outpath, dpi=200)
    plt.close(fig)
    print(f"Figura 4 salva em: {outpath}")


def main():
    parser = argparse.ArgumentParser(description="Geração das Figuras 3 e 4 do artigo")
    ...
    plot_fps_comparison(df, outdir / "figura3_fps_comparacao.png")
    plot_tradeoff(df, outdir / "figura4_tradeoff_map_fps.png")
